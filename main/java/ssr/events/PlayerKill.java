...
public class PlayerKill
{
	...
	public void killEvent(LivingDeathEvent event)
    {
        if (event.source.getEntity() instanceof EntityPlayerMP && event.entityLiving instanceof EntityLiving)
        {
            ...
            String localization = LanguageRegistry.instance().getStringLocalization(mobName);
            ...
            if ((EntityWhitelist.isEntityAccepted(mobName)) && !ent.getEntityData().getBoolean("fromSSR"))
            {
                if (!InvUtils.HasShard(player, mobName)) return;
    ...
    {
        return (stack == null ? 0 : EnchantmentHelper.getEnchantmentLevel(ObjHandler.sStealer.effectId, stack));
    }
}
