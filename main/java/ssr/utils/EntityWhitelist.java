...
public class EntityWhitelist 
{
	...
	{
		if (peacefuls.contains(entName) && !MobBlackList.bList.containsKey(entName)) return true;
		else if (!SoulConfig.disallowMobs && mobs.contains(entName) && !MobBlackList.bList.containsKey(entName)) return true;
		else return false;
	}
}
