...
public class CommandKillMobs extends CommandBase
{
	...
	public void processCommand(ICommandSender sender, String[] astring)
	{
		...
		while (iter.hasNext())
		{
			...
			if(ent instanceof EntityLiving && ent.getEntityData().getBoolean("fromSSR"))
			{
				ent.setDead();
				count++;
			}
		...
		if (count > 0)
			sender.addChatMessage(CreateMessage(1, "Remove "+count+" spawned entities."));
		else sender.addChatMessage(CreateMessage(2, "No spawned entities were found!"));
	}
	
	/**
	 * @param level 0 Default, 1 Success 2 Failure
	 * @param text
	 */
